angular.module('codenames', []).controller('appController', function($scope, $http, $timeout, $q) {
  ...
  _gameover = function(){
    ...
    $scope.title = $scope.title = "Game Over!!!";
  ...
  $scope.endTurn = function(){
    ...
    socket.emit('turn', $scope.turn, $scope.database);
  }
  ...
  socket.on('spymaster', function(masters, db){ //new spymaster added
    if(db == $scope.database) $scope.$apply(function(){ $scope.masters = masters; $scope.agents = $scope.players - $scope.masters;}); 
  });
  ...
  socket.on('players', function(players, db){ //win by death card
    if(db == $scope.database) $scope.$apply(function(){ $scope.players = players; $scope.agents = $scope.players - $scope.masters;});
  });
